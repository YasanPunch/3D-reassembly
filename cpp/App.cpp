...
MakePointCloud(int npts, const Eigen::Vector3d center, double radius,
               bool colorize) {
...
void MultiObjects() {
  ...
  visualization::Draw({pc_nocolor, pc_color, sphere_unlit, sphere_colored_unlit,
...
void Actions() {
  ...
  auto toggle_result = [TRUTH_NAME, RESULT_NAME](
                           visualization::visualizer::O3DVisualizer &o3dvis) {
...
Eigen::Matrix4d_u GetICPTransform(
  ...
  auto result = pipelines::registration::RegistrationICP(source, target,
                                                         max_dist, trans_init);
...
void Selections() {
  ...
  if (source->points_.empty()) {
    utility::LogError("Could not open {}", demo_icp_pointclouds.GetPaths(0));
    return;
  }
  ...
  if (target->points_.empty()) {
    utility::LogError("Could not open {}", demo_icp_pointclouds.GetPaths(1));
    return;
  }
  ...
  auto DoICPOneSet = [source, target, source_name, target_name](
                         visualization::visualizer::O3DVisualizer &o3dvis) {
    auto sets = o3dvis.GetSelectionSets();
    if (sets.empty()) {
      utility::LogWarning("You must select points for correspondence before "
                          "running ICP!");
      return;
    }
    ...
    std::vector<
        visualization::visualizer::O3DVisualizerSelections::SelectedIndex>
        source_picked(source_picked_set.begin(), source_picked_set.end());
    std::vector<
        visualization::visualizer::O3DVisualizerSelections::SelectedIndex>
        target_picked(target_picked_set.begin(), target_picked_set.end());
  ...
  auto DoICPTwoSets = [source, target, source_name, target_name](
                          visualization::visualizer::O3DVisualizer &o3dvis) {
    auto sets = o3dvis.GetSelectionSets();
    if (sets.size() < 2) {
      utility::LogWarning("You must have at least two sets of selected "
